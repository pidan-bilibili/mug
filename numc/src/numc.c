...
static PyObject *Matrix61c_add(Matrix61c* self, PyObject* args) {
    /* TODO: YOUR CODE HERE */
    if (!PyObject_TypeCheck(self, &Matrix61cType) || !PyObject_TypeCheck((Matrix61c *) args, &Matrix61cType)) {
        PyErr_SetString(PyExc_TypeError, "Invalid input to the function, not a Matrix61c Type");
        return NULL;
    }

    Matrix61c *result = (Matrix61c *) Matrix61c_new(&Matrix61cType, NULL, NULL);
    matrix *temp = NULL;
    matrix *mat0 = self->mat;
    matrix *mat1 = ((Matrix61c *) args)->mat;

    int alloc_code =  allocate_matrix(&temp, mat0->rows, mat0->cols);
    if (alloc_code == -1) {
        PyErr_SetString(PyExc_ValueError, "Rows & columns should be positive");
        return NULL;
    } else if (alloc_code == -2) {
        PyErr_SetString(PyExc_RuntimeError, "Allocation failed, unable to access memory");
        return NULL;
    }

    int add_code = add_matrix(temp, mat0, mat1);
    if (add_code == -1) {
        PyErr_SetString(PyExc_ValueError, "Rows & columns should be positive and equal");
        return NULL;
    } else if (add_code == -3) {
        PyErr_SetString(PyExc_ValueError, "Rows & columns should be equal");
        return NULL;
    }

    result->mat = temp;
    result->shape = PyTuple_Pack(2, PyLong_FromLong(temp->rows), PyLong_FromLong(temp->cols));
...
static PyObject *Matrix61c_sub(Matrix61c* self, PyObject* args) {
    ...
    int alloc_code =  allocate_matrix(&temp, mat0->rows, mat1->cols);
    if (alloc_code == -1) {
        PyErr_SetString(PyExc_ValueError, "Rows & columns should be positive");
        return NULL;
    } else if (alloc_code == -2) {
    ...
    int sub_code = sub_matrix(temp, mat0, mat1);
    if (sub_code == -1) {
        PyErr_SetString(PyExc_ValueError, "Rows & columns should be positive");
        return NULL;
    } else if (sub_code == -3) {
...
static PyObject *Matrix61c_multiply(Matrix61c* self, PyObject *args) {
    ...
    int mul_code = mul_matrix(temp, mat0, mat1);
    if (mul_code == -1) {
        PyErr_SetString(PyExc_ValueError, "Rows & columns should be positive");
        return NULL;
    } else if (mul_code == -3) {
...
static PyObject *Matrix61c_neg(Matrix61c* self) {
    ...
    int alloc_code = allocate_matrix(&temp, mat->rows, mat->cols);
    if (alloc_code == -1) {
        PyErr_SetString(PyExc_ValueError, "Rows & columns should be positive");
        return NULL;
    } else if (alloc_code == -2) {
    ...
    int neg_code = neg_matrix(temp, mat);
    if (neg_code == -1) {
        PyErr_SetString(PyExc_ValueError, "Rows & columns should be positive");
        return NULL;
    } else if (neg_code == -3) {
...
static PyObject *Matrix61c_abs(Matrix61c *self) {
    ...
    int abs_code = abs_matrix(temp, mat);
    if (abs_code == -1) {
        PyErr_SetString(PyExc_ValueError, "Rows & columns should be positive");
        return NULL;
    } else if (abs_code == -3) {
...
static PyObject *Matrix61c_pow(Matrix61c *self, PyObject *pow, PyObject *optional) {
    ...
    if (!PyObject_TypeCheck(self, &Matrix61cType) || !PyLong_Check(pow)) {
        PyErr_SetString(PyExc_TypeError, "Invalid input to the function, not a Matrix61c Type or Int Type");
        return NULL;
    }
    ...
    int pow_code = pow_matrix(temp, mat0, power);
    if (pow_code == -1) {
        PyErr_SetString(PyExc_ValueError, "Rows & columns should be positive");
        return NULL;
    } else if (pow_code == -2) {
        ...
        PyErr_SetString(PyExc_ValueError, "Input must be square matrix");
        return NULL;
    } else if (pow_code == -4) {
        PyErr_SetString(PyExc_ValueError, "Power should not be negative");
        return NULL;
    }
...
static PyObject *Matrix61c_set_value(Matrix61c *self, PyObject* args) {
    ...
    PyObject* arg2;
    PyObject* arg3;
    if (PyArg_UnpackTuple(args, "set", 3, 3, &arg1, &arg2, &arg3)) {
        if (!PyLong_Check(arg1) || !PyLong_Check(arg2)) {
            PyErr_SetString(PyExc_TypeError, "Arguments must be Integer");
            return NULL;
        }

        if (!PyLong_Check(arg3) && !PyFloat_Check(arg3)) {
            PyErr_SetString(PyExc_TypeError, "Argument must must be Integer or Float point");
            return NULL;
        }

        int row = (int) PyLong_AsLong(arg1);
        int col = (int) PyLong_AsLong(arg2);
        double val = (double) PyFloat_AsDouble(arg3);
...
static PyMethodDef Matrix61c_methods[] = {
    /* TODO: YOUR CODE HERE */
    {"get", (PyCFunction) Matrix61c_get_value, METH_VARARGS, "doc string"},
    {"set", (PyCFunction) Matrix61c_set_value, METH_VARARGS, "doc string"},
    {NULL, NULL, 0, NULL}
};
